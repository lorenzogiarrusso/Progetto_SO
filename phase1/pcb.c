 ...
 */
void initPcbs() {
    for(int i = 0; i < MAXPROC; i++) list_add(&(pcbTable[i].p_list), &pcbFree_h); //Inserisci ogni elemento dell'array nella lista di PCB liberi
}
...
pcb_t *allocPcb() {
    ...
    if(extracted == NULL) //Ovvero se la lista di PCB liberi e' vuota
        ...
        pcb_t *newPCB = container_of(extracted, struct pcb_t, p_list);
...
pcb_t *outProcQ(struct list_head *head, pcb_t *p) {
    pcb_t* iter;
    list_for_each_entry(iter, head, p_list){ //Itera sui PCB veri e propri nella lista
        if(iter == p) 
...
void insertChild(pcb_t *prnt, pcb_t *p) {
    p->p_parent = prnt;
    if(emptyChild(prnt)){ //prnt non ha figli, prnt->p_child deve puntare a p->p_sib
        list_add_tail(&(p->p_sib), &(prnt->p_child));
    }
    else {  //prnt ha gia' dei figli, devo inserire p in fondo alla coda di fratelli
        list_add_tail(&(p->p_sib), &(prnt->p_child));
    }
}
...
pcb_t *removeChild(pcb_t *p) {
    if(emptyChild(p))  //Ovvero p non ha figli
        ...
        pcb_t *firstChild = removeProcQ(&(p->p_child));
        return firstChild;
    }
}
